classdef perceptron_1 < handle
    properties
        l1
        rho_adaptatif
    end
    methods
        function init(obj,ne,ns)
            obj.l1 = couche;
            obj.l1.init(ne,ns);
            obj.rho_adaptatif=1;
        end
        ...
        function iteration(obj,c,data,itmax,rho)
            ...
            for i = 1:itmax
                Y2 = obj.l1.y(data);
                obj.l1.w = obj.l1.w -rho*obj.l1.grads(c,data,Y2);
                obj.l1.b = obj.l1.b -rho*obj.l1.gradbs(c,Y2);
        ...
        function retour=iterationscore(obj,c,data,itmax,rho,nbrfigure,titre)
            ...
            for i = 1:itmax
                ...
                if mod(i,100)==0
                    ...
                    score(iscore)=obj.pourcentage2(c,data);
                end
                waitbar(i/itmax,f,'Processing perceptron');
            end
            close (f);
        ...
        function retour = iterationscoreadaptatif(obj,c,data,itmax,nbrfigure,titre)
            ...
            iscore=0;
            N=size(data,1);
            lesf = [1/(2*N)*sum(sum(power(obj.l1.y(data)-c,2)))];
            for i = 1:itmax
                Y2 = obj.l1.y(data);
                w2=obj.l1.w;
                b2=obj.l1.b;
                obj.l1.w = obj.l1.w -obj.rho_adaptatif*obj.l1.grads(c,data,Y2);
                obj.l1.b = obj.l1.b -obj.rho_adaptatif*obj.l1.gradbs(c,Y2);
                lesf(i+1)=1/(2*N)*sum(sum(power(obj.l1.y(data)-c,2)));
                if (lesf(i+1)<=lesf(i))  
